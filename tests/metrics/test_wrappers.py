...
import sconce
from sconce.metrics import Std, Metric, Mean, BatchLambda, EpochLambda
...
class TestBatchLambda(unittest.TestCase):
    def setUp(self):
        ...
        self._states = [{sconce.Y_TRUE: Variable(torch.FloatTensor([1])), sconce.Y_PRED: Variable(torch.FloatTensor([2]))},
                        {sconce.Y_TRUE: Variable(torch.FloatTensor([3])), sconce.Y_PRED: Variable(torch.FloatTensor([4]))},
                        {sconce.Y_TRUE: Variable(torch.FloatTensor([5])), sconce.Y_PRED: Variable(torch.FloatTensor([6]))}]
    ...
    def test_train(self):
        ...
        for i in range(len(self._states)):
            ...
            calls.append(call(self._states[i][sconce.Y_PRED].data, self._states[i][sconce.Y_TRUE].data))
...
class TestEpochLambda(unittest.TestCase):
    def setUp(self):
        ...
        self._metric.reset({sconce.DEVICE: 'cpu', sconce.DATA_TYPE: torch.float32})
        self._states = [{sconce.BATCH: 0, sconce.Y_TRUE: torch.LongTensor([0]), sconce.Y_PRED: torch.FloatTensor([0.0]), sconce.DEVICE: 'cpu'},
                        {sconce.BATCH: 1, sconce.Y_TRUE: torch.LongTensor([1]), sconce.Y_PRED: torch.FloatTensor([0.1]), sconce.DEVICE: 'cpu'},
                        {sconce.BATCH: 2, sconce.Y_TRUE: torch.LongTensor([2]), sconce.Y_PRED: torch.FloatTensor([0.2]), sconce.DEVICE: 'cpu'},
                        {sconce.BATCH: 3, sconce.Y_TRUE: torch.LongTensor([3]), sconce.Y_PRED: torch.FloatTensor([0.3]), sconce.DEVICE: 'cpu'},
                        {sconce.BATCH: 4, sconce.Y_TRUE: torch.LongTensor([4]), sconce.Y_PRED: torch.FloatTensor([0.4]), sconce.DEVICE: 'cpu'}]
