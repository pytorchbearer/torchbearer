...
import sconce
from sconce.callbacks import TorchScheduler, LambdaLR, StepLR, MultiStepLR, ExponentialLR, CosineAnnealingLR,\
    ReduceLROnPlateau


class TestTorchScheduler(TestCase):
    def test_torch_scheduler_on_batch_with_monitor(self):
        state = {sconce.EPOCH: 1, sconce.METRICS: {'test': 101}, sconce.OPTIMIZER: 'optimizer'}
        mock_scheduler = Mock()
    ...
    def test_torch_scheduler_on_batch_no_monitor(self):
        state = {sconce.EPOCH: 1, sconce.OPTIMIZER: 'optimizer'}
        mock_scheduler = Mock()
        mock_scheduler.return_value = mock_scheduler
...
class TestLambdaLR(TestCase):
    ...
    def test_lambda_lr(self, lr_mock):
        state = {sconce.OPTIMIZER: 'optimizer'}
