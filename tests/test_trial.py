...
class TestTrial(TestCase):
    ...
    def test_fit_valid_sets_args(self, gtvs):
        ...
        torchtrial = MagicMock()
        torchtrial.forward = Mock(return_value=1)
        optimizer = MagicMock()
        metric = Metric('test')
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearertrial.with_train_data(x,y,1,shuffle=shuffle)
        torchbearertrial.with_val_data(val_data[0], val_data[1],batch_size=1,shuffle=shuffle)

        torchbearertrial.fit(x, y, 1, validation_data=val_data, validation_split=val_split, shuffle=shuffle)
    ...
    def test_fit_no_valid(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearertrial.fit_generator = Mock()
        fit = torchbearertrial.fit_generator
        torchbearertrial.fit(x, y, 1, validation_split=None)
    ...
    def test_main_loop_metrics(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearerstate = torchbearertrial.fit_generator(generator, train_steps, epochs, 0, [callback], initial_epoch=0, pass_state=False)
    ...
    def test_main_loop_verbose(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearerstate = torchbearertrial.fit_generator(generator, train_steps, epochs, 1, [callback], initial_epoch=0, pass_state=False)
    ...
    def test_main_loop_train_steps_positive(self):
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == train_steps)
    ...
    def test_main_loop_train_steps_fractional(self, _):
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == int(train_steps))
    ...
    def test_main_loop_validation_setup(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearertrial._test_pass = Mock()
        torchbearerstate = torchbearertrial.fit_generator(generator, train_steps, epochs, 0, [callback],
    ...
    def test_main_loop_epochs_positive(self):
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == len(data)*epochs)
    ...
    def test_main_loop_epochs_negative(self):
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == 0)
    ...
    def test_main_loop_epochs_fractional(self, _):
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == int(epochs)*len(data))
    ...
    def test_main_loop_train_steps_too_big(self):
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == len(data))
    ...
    def test_main_loop_pass_state(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearerstate = torchbearertrial.fit_generator(generator, train_steps, epochs, 0, [callback], initial_epoch=0, pass_state=True)

        self.assertTrue(len(torchbearerstate[torchbearer.TRIAL].call_args) == 2)
    ...
    def test_main_loop_criterion(self):
        ...
        state = torchbearertrial.main_state.copy()
        torchbearerstate = torchbearertrial.fit_generator(generator, train_steps, epochs, 0, [callback], initial_epoch=0, pass_state=True)
    ...
    def test_main_loop_stop_training(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearerstate = torchbearertrial.fit_generator(generator, train_steps, epochs, 0, [callback], initial_epoch=0, pass_state=True)
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == 1)
    ...
    def test_test_loop_metrics(self):
        ...
        state.update({torchbearer.METRIC_LIST: metric_list, torchbearer.VALIDATION_GENERATOR: validation_generator,
                 torchbearer.CallbackList: callback_List, torchbearer.TRIAL: torchtrial, torchbearer.VALIDATION_STEPS: validation_steps,
                 torchbearer.CRITERION: criterion, torchbearer.STOP_TRAINING: False, torchbearer.METRICS: {}})

        torchbearerstate = torchbearertrial._test_pass(state, callback_List, False, Trial._load_batch_standard, num_steps=None)
    ...
    def test_test_loop_forward(self):
        ...
        torchtrial = Mock(return_value=1)
        optimizer = MagicMock()
        ...
        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == 3)
    ...
    def test_test_loop_pass_state(self):
        ...
        torchbearerstate = torchbearertrial._test_pass(state, callback_List, True, Trial._load_batch_standard, num_steps=None)
    ...
    def test_test_loop_num_steps_positive(self):
        ...
        torchbearerstate = torchbearertrial._test_pass(state, callback_List, False, Trial._load_batch_standard, num_steps=validation_steps)

        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == 2)
    ...
    def test_test_loop_none_gen(self):
        ...
        torchbearerstate = torchbearertrial._test_pass(state, callback_List, False, Trial._load_batch_none, num_steps=validation_steps)

        self.assertTrue(torchbearerstate[torchbearer.TRIAL].call_count == validation_steps)
    ...
    def test_evaluate(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearertrial.evaluate_generator = Mock()
        ev = torchbearertrial.evaluate_generator
        torchbearertrial.evaluate(x, y, verbose=verbose, pass_state=pass_state)
    ...
    def test_evaluate_generator_args(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, torch.nn.L1Loss(), [])
        torchbearertrial.main_state[torchbearer.METRICS] = 1
        torchbearertrial._test_pass = Mock()

        torchbearertrial.evaluate_generator(generator, 0, steps, pass_state)
        self.assertTrue(torchbearertrial._test_pass.call_args[0][1].callback_list == [])
        self.assertTrue(torchbearertrial._test_pass.call_args[0][2] == pass_state)
        self.assertTrue(torchbearertrial._test_pass.call_args[0][4] == steps)
    ...
    def test_evaluate_generator_verbose(self):
        ...
        torchbearertrial.evaluate_generator(generator, 1, steps, pass_state)
        self.assertIsInstance(torchbearertrial._test_pass.call_args[0][1].callback_list[0], Tqdm)
    ...
    def test_predict(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, criterion, [metric])
        torchbearertrial.predict_generator = Mock()
        pred = torchbearertrial.predict_generator
        torchbearertrial.predict(x, verbose=verbose, pass_state=pass_state)
    ...
    def test_predict_generator_args(self):
        ...
        torchbearertrial = Trial(torchtrial, optimizer, torch.nn.L1Loss(), [])
        torchbearertrial.main_state[torchbearer.FINAL_PREDICTIONS] = 1
        torchbearertrial._test_pass = Mock()

        torchbearertrial.predict_generator(generator, 0, steps, pass_state)
        self.assertIsInstance(torchbearertrial._test_pass.call_args[0][1].callback_list[0], AggregatePredictions)
        self.assertTrue(torchbearertrial._test_pass.call_args[0][2] == pass_state)
        self.assertTrue(torchbearertrial._test_pass.call_args[0][4] == steps)
    ...
    def test_predict_generator_verbose(self):
        ...
        torchbearertrial.predict_generator(generator, 1, steps, pass_state)
        self.assertIsInstance(torchbearertrial._test_pass.call_args[0][1].callback_list[0], Tqdm)
        self.assertTrue(torchbearertrial._test_pass.call_args[0][2] == pass_state)
        self.assertTrue(torchbearertrial._test_pass.call_args[0][4] == steps)
    ...
    def test_train(self):
        torchtrial = torch.nn.Sequential(torch.nn.Linear(1,1))
        optimizer = MagicMock()
        metric_list = MagicMock()

        torchbearertrial = Trial(torchtrial, optimizer, torch.nn.L1Loss(), [])
        torchbearertrial.main_state = {torchbearer.TRIAL: torchtrial, torchbearer.METRIC_LIST: metric_list}
        torchbearertrial.train()
        self.assertTrue(torchbearertrial.main_state[torchbearer.TRIAL].training == True)
        torchbearertrial.main_state[torchbearer.METRIC_LIST].train.assert_called_once()
    ...
    def test_eval(self):
        ...
        self.assertTrue(torchbearertrial.main_state[torchbearer.TRIAL].training == False)
        torchbearertrial.main_state[torchbearer.METRIC_LIST].eval.assert_called_once()
    ...
    def test_to_both_args(self):
        ...
        torchtrial = torch.nn.Sequential(torch.nn.Linear(1,1))
        torchtrial.to = Mock()
        optimizer = torch.optim.Adam(torchtrial.parameters(), 0.1)
        ...
        self.assertTrue(torchtrial.to.call_args[0][0] == dev)
        self.assertTrue(torchtrial.to.call_args[0][1] == dtype)
    ...
    def test_to_only_dtype(self):
        ...
        self.assertTrue(torchtrial.to.call_args[0][0] == dtype)
    ...
    def test_to_kwargs(self):
        ...
        self.assertTrue(torchtrial.to.call_args[1]['device'] == dev)
        self.assertTrue(torchtrial.to.call_args[1]['dtype'] == dtype)
    ...
    def test_update_device_and_dtype_from_args_only_kwarg(self):
        ...
        main_state = Trial._update_device_and_dtype_from_args(main_state, **kwargs)
    ...
    def test_update_device_and_dtype_from_args_only_arg(self):
        ...
        main_state = Trial._update_device_and_dtype_from_args(main_state, *args)
    ...
    def test_cuda_no_device(self, device_mock):
        ...
        torchtrial = torch.nn.Sequential(torch.nn.Linear(1,1))
        torchtrial.load_state_dict = Mock()

        optimizer = torch.optim.SGD(torchtrial.parameters(), 0.1)
    ...
    def test_load_state_dict(self):
        ...
        torchtrial = torch.nn.Sequential(torch.nn.Linear(1,1))
        torchtrial.load_state_dict = Mock()
        torch_state = torchtrial.state_dict()
        ...
        self.assertTrue(torchtrial.load_state_dict.call_args[0][0] == torch_state)
        self.assertTrue(optimizer.load_state_dict.call_args[0][0] == optimizer_state)
        self.assertTrue(torchtrial.load_state_dict.call_args[1] == key_words)

    def test_state_dict(self):
        torchtrial = torch.nn.Sequential(torch.nn.Linear(1,1))
        torchtrial_state = torchtrial.state_dict()
        ...
        self.assertTrue(torchbearer_state[torchbearer.TRIAL] == torchtrial_state)
    ...
    def test_state_dict_kwargs(self):
        ...
        self.assertTrue(torchtrial.state_dict.call_args[1] == keywords)
    ...
    def test_deep_to_tensor(self):
        ...
        Trial._deep_to(tensor, new_device, new_dtype)
    ...
    def test_deep_to_list(self):
        ...
        Trial._deep_to(tensors, new_device, new_dtype)
    ...
    def test_load_batch_standard(self):
        ...
        Trial._load_batch_standard('training', state)
    ...
    def test_load_batch_predict_data(self):
        ...
        iterator = iter(items)
        state = {'training_iterator': iterator, 'device': 'cpu', 'dtype': torch.int}
        Trial._load_batch_predict('training', state)
