...
torchbearer_model = Trial(model, optimizer, loss, metrics=['acc', 'loss'], callbacks=[TensorBoard(visdom=True, write_graph=True, write_batch_metrics=True, batch_step_size=10, write_epoch_metrics=True)]).to('cuda')
torchbearer_model.with_generators(train_generator=traingen, val_generator=valgen)
torchbearer_model.run(epochs=5)

import torchbearer.callbacks.tensor_board as tensorboard

tensorboard.VisdomParams.ENV = 'Test'
torchbearer_model = Trial(model, optimizer, loss, metrics=['acc', 'loss'], callbacks=[TensorBoard(visdom=True, write_graph=False, write_batch_metrics=True, batch_step_size=10, write_epoch_metrics=False)]).to('cuda')
torchbearer_model.with_generators(train_generator=traingen, val_generator=valgen)
torchbearer_model.run(epochs=5)
