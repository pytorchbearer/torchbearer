...
def gaussian_rbf(u, v, gamma):
    return torch.exp(- torch.sqrt(((u.unsqueeze(1) - v.unsqueeze(0)) ** 2).sum(2) + (gamma**2))).t()
...
class NonLinearSVM(nn.Module):
    ...
    def __init__(self, batch_size=32):
        ...
        self.u = nn.Parameter(torch.randn(1, batch_size), requires_grad=True)
        self.b = nn.Parameter(torch.randn(1), requires_grad=True)

    def forward(self, x):
        h = gaussian_rbf(x, x).matmul(self.w.t()) + self.b
        return h
