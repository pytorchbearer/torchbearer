...
This guide will give a quick intro to training PyTorch models with PySconce. We'll start by loading in some data and defining a model, then we'll train it for a few epochs and see how well it does.

Defining the Model
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Let's get using sconce. Here's some data from Cifar10 and a simple 3 layer strided CNN:
...
Instead, with sconce, we can define our optimiser and some metrics (just 'acc' and 'loss' for now) and let it do the work.
