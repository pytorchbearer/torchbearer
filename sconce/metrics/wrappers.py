...
class BatchLambda(metrics.Metric):
    ...
    def process(self, state):
        ...
        """
        return self._metric_function(state[sconce.Y_PRED], state[sconce.Y_TRUE])
...
class EpochLambda(metrics.AdvancedMetric):
    ...
    def process_train(self, state):
        ...
        """
        self._y_true = torch.cat((self._y_true, state[sconce.Y_TRUE]), dim=0)
        self._y_pred = torch.cat((self._y_pred, state[sconce.Y_PRED].float()), dim=0)
        if state[sconce.BATCH] % self._step_size == 0:
    ...
    def process_validate(self, state):
        ...
        """
        self._y_true = torch.cat((self._y_true, state[sconce.Y_TRUE]), dim=0)
        self._y_pred = torch.cat((self._y_pred, state[sconce.Y_PRED].to(self._y_pred.dtype)), dim=0)
    ...
    def reset(self, state):
        ...
        self._y_true = self._y_true.to(state[sconce.DEVICE])
        self._y_pred = self._y_pred.to(state[sconce.DEVICE], state[sconce.DATA_TYPE])
