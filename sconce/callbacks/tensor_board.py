...
class TensorBoard(Callback):
    ...
    def __init__(self, log_dir='./logs',
        ...
        if self.write_graph:
            def handle_graph(state):
                dummy = torch.rand(state[sconce.X].size(), requires_grad=False)
                model = copy.deepcopy(state[sconce.MODEL]).to('cpu')
    ...
    def on_start(self, state):
        self.log_dir = os.path.join(self.log_dir, state[sconce.MODEL].__class__.__name__ + '_' + self.comment)
        self._writer = SummaryWriter(log_dir=self.log_dir)

    def on_start_epoch(self, state):
        if self.write_batch_metrics:
            log_dir = os.path.join(self.log_dir, 'epoch-' + str(state[sconce.EPOCH]))
    ...
    def on_step_training(self, state):
        if self.write_batch_metrics and state[sconce.BATCH] % self.batch_step_size == 0:
            for metric in state[sconce.METRICS]:
                self._batch_writer.add_scalar('batch/' + metric, state[sconce.METRICS][metric], state[sconce.BATCH])
    ...
    def on_end_epoch(self, state):
        ...
        if self.write_epoch_metrics:
            for metric in state[sconce.METRICS]:
                self._writer.add_scalar('epoch/' + metric, state[sconce.METRICS][metric], state[sconce.EPOCH])
...
class TensorBoardImages(Callback):
    ...
    def on_start(self, state):
        log_dir = os.path.join(self.log_dir, state[sconce.MODEL].__class__.__name__ + '_' + self.comment)
    ...
    def on_step_validation(self, state):
        if not self.done:
            ...
            if self._data.size(0) >= self.num_images:
                ...
                self._writer.add_image(self.name, image, state[sconce.EPOCH])
                self.done = True
                self._data = None
...
class TensorBoardProjector(Callback):
    ...
    def on_step_validation(self, state):
        if not self.done:
            ...
            if state[sconce.EPOCH] == 0 and self.write_data:
            ...
            label = state[sconce.Y_TRUE].data.clone()
            ...
            if self._labels.size(0) >= self.num_images:
                ...
                if self.write_features:
                    self._writer.add_embedding(self._features, metadata=self._labels, label_img=self._images, tag='features', global_step=state[sconce.EPOCH])
                self.done = True
